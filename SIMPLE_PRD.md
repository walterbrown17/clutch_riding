...
| Cycle ID | Cycle Start Time | Cycle End Time | Mileage Degradation | Potential Savings (INR) |
| :--- | :--- | :--- | :--- | :--- |
| cycle-xyz-001 | 08:15 AM | 09:20 AM | 35.2% | ₹ 85.50 |
| cycle-xyz-002 | 11:30 AM | 12:05 PM | 15.8% | ₹ 42.10 |
| cycle-abc-001 | 04:45 PM | 05:30 PM | 41.0% | ₹ 105.20 |
| ... | ... | ... | ... | ... |
...
```sql
-- This query generates the "Cycle-Level Drill-Down" report. It is designed
-- to be executed on-the-fly and only shows cycles where clutch riding led
-- to a measurable degradation in mileage.
...
WITH cycle_agg AS (
    -- Aggregate all event data for each cycle in a single pass
    SELECT
        cycle_id,
...
final_report_data AS (
    -- Calculate final metrics like mileage and degradation from the aggregated data
    SELECT
        cycle_id,
        cycle_start_ts,
        cycle_end_ts,
        clutch_distance_km,
        clutch_fuel_liters,
        -- Calculate mileage, avoiding division by zero
        (CASE WHEN normal_fuel_liters > 0 THEN normal_distance_km / normal_fuel_liters ELSE 0 END) AS normal_mileage_kmpl,
        (CASE WHEN clutch_fuel_liters > 0 THEN clutch_distance_km / clutch_fuel_liters ELSE 0 END) AS clutch_mileage_kmpl
    FROM
        cycle_agg
    WHERE
        clutch_fuel_liters > 0 -- Ensure the cycle had clutch riding events
),

-- Final Formatting and Filtering step
formatted_report AS (
    SELECT
        cycle_id AS "Cycle ID",
        TO_CHAR(TO_TIMESTAMP(cycle_start_ts), 'HH:MI AM') AS "Cycle Start Time",
        TO_CHAR(TO_TIMESTAMP(cycle_end_ts), 'HH:MI AM') AS "Cycle End Time",
...
-- Filter for cycles with actual mileage degradation
SELECT *
FROM formatted_report
WHERE "Mileage Degradation" > 0
ORDER BY "Cycle Start Time" DESC;
```
### 7.4.1 SQL Generation Logic
